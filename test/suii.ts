...
describe("LiquidityExamples", () => {
  ...
  before(async () => {
    ...
    const daiAmount = BigInt("100000000000");
    const usdcAmount = BigInt("1000000");
    console.log(
      "DAI Whale balance : ",
      (await dai.balanceOf(daiWhale.address)).toString()
    );
    console.log(
      "USDC Whale balance :",
  ...
  it("mintNewPosition", async () => {
    const daiAmount = BigInt("1000000000");
    const usdcAmount = BigInt("1000");
    await dai
      .connect(accounts[0])
      .approve(liquidityExamples.target, ethers.MaxUint256);
    await usdc
      .connect(accounts[0])
      .approve(liquidityExamples.target, ethers.MaxUint256);
    ...
    await dai
      .connect(accounts[0])
      .transfer(liquidityExamples.target, daiAmount);
    console.log("Pass 5");
    await usdc
      .connect(accounts[0])
      .transfer(liquidityExamples.target, usdcAmount);
    console.log("Pass 6");

    const daiBal = await liquidityExamples.getDAIBalance(accounts[0]);
    const usdcBal = await liquidityExamples.getUSDCBalance(accounts[0]);

    console.log(daiBal,usdcBal);
